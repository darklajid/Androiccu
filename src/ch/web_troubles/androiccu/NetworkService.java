...
public class NetworkService extends Service {
		...
		try {
			...
			if (networkInfo != null) {
				...
				if (networkConnected) {
					...
					log("waiting to be connected to " + networkTypeName
							+ "network");
				}
			} else {
	...
	private void log(String message) {
		...
		try {
			...
			outputStream = openFileOutput("log", Context.MODE_PRIVATE
					| Context.MODE_APPEND);
			...
			if (intent.getAction().equals(
				...
				try {
					...
					if (networkInfo == null) { // Completely lost any network
						...
						if (oldNetworkType != networkType) { // Change of
																// network,
																// might need to
																// stop and/or
																// start aiccu
						...
						} else if (oldNetworkConnected != networkConnected) { // Network
																				// connection
																				// change
							if (networkConnected) {
					...
					if (networkType == ConnectivityManager.TYPE_WIFI) {
						...
						if (info.getState() == NetworkInfo.State.CONNECTED) {
							if (!networkConnected) {
								...
								log("new wifi connection"
										+ ((wifiSSID.length() > 0) ? (" to " + wifiSSID)
												: ""));
								startAiccu(false);
							}
						} else {
				...
				setStatusIntent.putExtra(MainActivity.EXTRA_STATUS,
						aiccuStarted);
				sendBroadcast(setStatusIntent);
			}
		}
