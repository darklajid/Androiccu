...
public class MainActivity extends Activity {
	...
	public void onResume() {
		...
		try {
			...
			if (previousVersion < getPackageManager().getPackageInfo(
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menu_install:
			if (running) {
				popup("Warning",
						getResources().getString(R.string.menu_install_warning));
				return true;
			}
		...
		case R.id.menu_config:
			if (running) {
				popup("Warning",
						getResources().getString(R.string.menu_config_warning));
				return true;
			}
		...
		if (!installed) {
			...
			if (running) {
				textView.setText(getResources()
						.getString(R.string.main_running) + "\n" + log);
			} else {
				textView.setText(getResources().getString(
						R.string.main_not_running)
						+ "\n" + log);
			}
		}
	...
	private class CheckTask extends AsyncTask<Void, Void, Boolean> {
		protected Boolean doInBackground(Void... dummy) {
			try {
				...
				Vector<String> cmd_ret = shell.exec("sh",
						"ls /system/xbin/aiccu");
				...
				cmd_ret = shell.exec("sh", "ls " + dataDir.getPath()
						+ "/aiccu/aiccu.conf");
				...
				for (RunningServiceInfo service : manager
					...
					if (NetworkService.class.getName().equals(
